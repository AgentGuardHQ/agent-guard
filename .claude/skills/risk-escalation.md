...
```bash
cat logs/runtime-events.jsonl 2>/dev/null | tail -200 | grep -c '"policy_result":"deny"' 2>/dev/null
cat logs/runtime-events.jsonl 2>/dev/null | tail -200 | wc -l 2>/dev/null
```
...
```bash
gh issue list --state open --label "source:risk-escalation" --label "priority:critical" --json number --jq '.[0].number' 2>/dev/null
```
...
*Auto-created by risk-escalation agent. This alert requires human attention.*" \
  --label "source:risk-escalation" --label "priority:critical" --label "status:pending"
```
...
```bash
gh issue list --state open --label "source:risk-escalation" --json number,labels --jq '[.[] | select(.labels | map(.name) | contains(["priority:critical"]) | not)] | .[0].number' 2>/dev/null
```
...
gh issue create \
  ...
  --label "source:risk-escalation" --label "status:pending"
```
