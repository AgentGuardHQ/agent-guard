...
```
completion_ratio = checked_items / total_items  (from ROADMAP.md checkboxes)
...
*Auto-created by progress-controller on $(date -u +%Y-%m-%dT%H:%M:%SZ)*" \
  --label "source:progress-controller" --label "status:pending"
```
...
- `currentPhase`: the active phase name/number derived from ROADMAP.md
