...
export function drawBattle(
  ...
  drawHPBar(ctx, 300, 110, 100, battle.enemy.currentHP, battle.enemy.hp);
  ...
  if (!playerMon.sprite || !drawSprite(ctx, playerMon.sprite, 80, 140, 64, 64)) {
    const playerSprite = generateMonster(playerMon.id, playerMon.color || '#3498db', 64);
    ctx.drawImage(playerSprite, 80, 140);
  }
  ctx.fillStyle = '#fff';
  ctx.fillText(playerMon.name, 60, 130);
  drawHPBar(ctx, 60, 210, 100, playerMon.currentHP, playerMon.hp);
